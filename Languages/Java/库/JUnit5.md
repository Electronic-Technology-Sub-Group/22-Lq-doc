...
- 测试方法：使用以下方法标记的方法即测试方法，测试方法一不定是 private 的，且注解都可以被继承
...
需要与 `TestTemplateInvocationContextProvider` 搭配使用
## 动态测试

使用 `@TestFactory` 工厂方法在运行时生成测试方法的测试称为动态测试。`@TestFactory` 修饰的方法必须满足以下几点：

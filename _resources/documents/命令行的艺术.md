...
title: "the-art-of-command-line/README-zh.md at master · jlevy/the-art-of-command-line"
...
image: https://repository-images.githubusercontent.com/35955666/03420800-9e9b-11ea-8913-de5564db65c9
```
...
![[../images/Pasted image 20240905012706.png]]
...
- 在 Bash 中，变量有许多的扩展方式。
	...
	- 只需要一个参数时，可以使用 `input_file=${1:?usage: $0 input_file}`。
	- 在变量为空时使用默认值：`${name:-default}`。
	- 在之前的例子中再加一个（可选的）参数 `output_file=${2:-logfile}`
		- 如果省略了 `$2`，值为空，`output_file` 就会被设为 `logfile`。
	- 数学表达式：`i=$(( (i + 1) % 5 ))`。
	- 序列：`{1..10}`。
	- 截断字符串：`${var%suffix}` 和 `${var#prefix}`。
		- 假设 `var=foo.pdf`，那么 `echo ${var%.pdf}.txt` 将输出 `foo.txt`。
...
- 在当前目录下通过文件名查找一个文件：`find . -iname '*something*'`。
	- 在所有路径下通过文件名查找文件：`locate something`
	- `updatedb` 可能没有对最近新建的文件建立索引，所以你可能无法定位到这些未被索引的文件。

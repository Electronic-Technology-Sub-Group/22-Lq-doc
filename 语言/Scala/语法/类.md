...
默认所有成员为 public，使用 private 修饰的成员可成为私有成员
...
Case Class，不可变的类，使用 `case class` 定义，创建对象时无需 `new` 关键字，常用于对不可变数据的建模
...
```scala title:gardening/fruits/package.scala
// 包对象
// 可混入/继承
package gardening
